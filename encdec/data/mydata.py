...
class MyData(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = [self.normalize(cv2.imread(os.path.join(self.root, self.split, im_dir, name), cv2.IMREAD_UNCHANGED)) for im_dir in self.img_dirs]
        ...
        label = [self.normalize(cv2.imread(os.path.join(self.root, self.split, lbl_dir, name), cv2.IMREAD_UNCHANGED)) for lbl_dir in self.lbl_dirs]
